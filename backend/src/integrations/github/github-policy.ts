...
const DEFAULT_POLICY_CHECK_NAME = "PR Agent Policy";
...
async function publishPolicyReminderComment(params: {
  ...
  const body = [
    localizeText(
      { zh: "## PR Agent 流程守卫", en: "## PR Agent Flow Guard" },
      params.locale,
    ),
    "",
    `**${title}**`,
    ...
    localizeText(
      {
        zh: "可在仓库根目录 `.pr-agent.yml` 调整规则（`remind` / `enforce`）。",
        en: "You can tune these rules in `.pr-agent.yml` (`remind` / `enforce`).",
      },
      params.locale,
    ),
  ].join("\n");
