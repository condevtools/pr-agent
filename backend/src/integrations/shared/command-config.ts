...
export function buildConfigFoundMessage(params: {
  ...
  const header = localizeText(
    {
      zh: "### 当前仓库配置（`.pr-agent.yml`）",
      en: "### Current Repository Config (`.pr-agent.yml`)",
    },
    locale,
  );
...
export function buildConfigNotFoundMessage(locale: UiLocale): string {
  return localizeText(
    {
      zh: "当前仓库未找到 `.pr-agent.yml` 配置文件，使用默认配置。可参阅 README 了解可配置项。",
      en: "No `.pr-agent.yml` config file found in this repository. Using default settings. See README for available options.",
    },
    locale,
  );
}
