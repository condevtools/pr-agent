...
const DEFAULT_WEBHOOK_EVENT_STORE_FILE = ".pr-agent-webhook-events.ndjson";
...
async function persistStoredWebhookEventBestEffortAsync(event: StoredWebhookEvent): Promise<void> {
  try {
    ...
    incrementMetricCounter("pr_agent_webhook_store_writes_total", {
...
export function assertWebhookReplayAuthorized(
  ...
  const actual = readHeaderValue(headers, "x-pr-agent-replay-token")?.trim();
  if (!actual) {
...
async function trimStoredWebhookEventsAsync(): Promise<void> {
  ...
  incrementMetricCounter("pr_agent_webhook_store_trim_total", {
    result: "trimmed",
  });
}
