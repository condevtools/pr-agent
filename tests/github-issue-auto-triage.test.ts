...
function withIssueAutoTriageEnv(
  ...
  const original = {
    MR_AGENT_LOCALE: process.env.MR_AGENT_LOCALE,
  ...
  return fn().finally(() => {
    process.env.MR_AGENT_LOCALE = original.MR_AGENT_LOCALE;
...
test("github issues webhook auto-triages issue and labels when .mr-agent.yml is missing", async () => {
...
test("github issues webhook skips auto-triage when .mr-agent.yml exists", async () => {
  clearDuplicateRequestState();
  const postedComments: string[] = [];
  const postedLabels: string[][] = [];
