...
function buildStorePath(label: string): string {
  return join(
    "/tmp",
    `mr-agent-webhook-events-${label}-${Date.now()}-${Math.random().toString(16).slice(2)}.ndjson`,
  );
}
...
test("webhook request metrics collapse unknown event labels to other", async () => {
  ...
  assert.match(
    metrics,
    /mr_agent_webhook_requests_total\{event="other",platform="github"\} 1/,
  );
  assert.match(
    metrics,
    /mr_agent_webhook_requests_total\{event="other",platform="gitlab"\} 1/,
  );
});
...
test("metrics counter store caps cardinality to avoid unbounded memory growth", () => {
  ...
  try {
    ...
    assert.doesNotMatch(
      rendered,
      /mr_agent_webhook_requests_total\{event="e0",platform="github"\}/,
    );
    assert.match(
      rendered,
      /mr_agent_webhook_requests_total\{event="e1",platform="github"\} 1/,
    );
    assert.match(
      rendered,
      /mr_agent_webhook_requests_total\{event="e100",platform="github"\} 1/,
    );
  } finally {
