...
const lockStateFile = join(
  "/tmp",
  `mr-agent-runtime-state-lock-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
);
...
test("dedupe state survives module reload via runtime state backend", async () => {
  ...
  try {
    const runtime = await import(`@mr-agent/core/runtime-state.ts?runtime_a=${Date.now()}`);
    await runtime.prepareRuntimeStateBackend();
    runtime.clearRuntimeStateStore();
    const modA = await import(`@mr-agent/core/dedupe.ts?dedupe_a=${Date.now()}`);
    const modB = await import(`@mr-agent/core/dedupe.ts?dedupe_b=${Date.now()}`);
...
test("rate-limit state survives module reload via runtime state backend", async () => {
  ...
  try {
    const runtime = await import(`@mr-agent/core/runtime-state.ts?runtime_b=${Date.now()}`);
    await runtime.prepareRuntimeStateBackend();
    runtime.clearRuntimeStateStore();
    const modA = await import(`@mr-agent/core/rate-limit.ts?rate_a=${Date.now()}`);
    const modB = await import(`@mr-agent/core/rate-limit.ts?rate_b=${Date.now()}`);
...
test("ask conversation state survives module reload via runtime state backend", async () => {
  ...
  try {
    const runtime = await import(`@mr-agent/core/runtime-state.ts?runtime_c=${Date.now()}`);
    await runtime.prepareRuntimeStateBackend();
    runtime.clearRuntimeStateStore();
    const modA = await import(`@mr-agent/core/ask-session.ts?ask_a=${Date.now()}`);
    const modB = await import(`@mr-agent/core/ask-session.ts?ask_b=${Date.now()}`);
...
test("runtime state backend supports sqlite persistence", async () => {
  ...
  try {
    const runtimeA = await import(`@mr-agent/core/runtime-state.ts?runtime_sqlite_a=${Date.now()}`);
    ...
    const runtimeB = await import(`@mr-agent/core/runtime-state.ts?runtime_sqlite_b=${Date.now()}`);
...
test("runtime state sqlite backend fails fast on initialization errors", async () => {
  ...
  try {
    const runtime = await import(
      `@mr-agent/core/runtime-state.ts?runtime_sqlite_failfast=${Date.now()}`
    );
...
test("runtime state backend readiness check fails fast for sqlite init errors", async () => {
  ...
  try {
    const runtime = await import(
      `@mr-agent/core/runtime-state.ts?runtime_sqlite_ready_failfast=${Date.now()}`
    );
    assert.throws(
...
test("runtime state file backend persists after temporary file lock contention", async () => {
  ...
  try {
    const runtimeA = await import(`@mr-agent/core/runtime-state.ts?runtime_file_lock_a=${Date.now()}`);
    await runtimeA.prepareRuntimeStateBackend();
    runtimeA.clearRuntimeStateStore();
    ...
    const runtimeB = await import(`@mr-agent/core/runtime-state.ts?runtime_file_lock_b=${Date.now()}`);
    await runtimeB.prepareRuntimeStateBackend();
