...
import { mergeChangelogContent } from "@mr-agent/shared/changelog.ts";
import {
  buildAddDocRule,
  buildChecksQuestion,
...
} from "@mr-agent/shared/command-builders.ts";
import { buildDescribeQuestion } from "@mr-agent/shared/describe-question.ts";
import { getPublicErrorMessage } from "@mr-agent/shared/public-error.ts";
import {
  parseYamlBooleanMaybe,
  stripYamlQuotes,
} from "@mr-agent/shared/yaml.ts";
import {
  findPotentialSecrets,
  isLikelyPlaceholder,
} from "@mr-agent/shared/secret-scan.ts";
import { inferReviewLabels } from "@mr-agent/shared/auto-labels.ts";
import { buildSecretWarningComment } from "@mr-agent/shared/secret-warning.ts";
import {
  buildSimilarIssueComment,
  buildSimilarIssueQueryMissingMessage,
  resolveSimilarIssueQuery,
} from "@mr-agent/shared/similar-issue.ts";
