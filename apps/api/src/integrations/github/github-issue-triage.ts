...
import { resolveSimilarIssueQuery } from "@mr-agent/shared/similar-issue.js";
...
function buildIssueAutoTriageComment(params: {
  ...
  return [
    ...
    localizeText(
      {
        zh: "检测到仓库未配置 `.mr-agent.yml/.mr-agent.yaml`，已使用默认策略自动给出分诊建议。",
        en: "No `.mr-agent.yml/.mr-agent.yaml` detected. Applied default triage policy automatically.",
      },
      params.locale,
    ),
    "",
    ...
    localizeText(
      {
        zh: "可在仓库根目录新增 `.mr-agent.yml` 来覆盖默认行为。",
        en: "Add `.mr-agent.yml` at repository root to customize this behavior.",
      },
      params.locale,
    ),
  ].join("\n");
